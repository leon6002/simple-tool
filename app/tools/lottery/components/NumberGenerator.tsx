...
import {
  calculateDLTBetAmount,
  calculateSSQBetAmount,
  calculateKL8BetAmount,
  calculateKL8BetCount,
  calculateBetCount,
  formatCurrency,
  validateKL8BetLimit,
} from "../constants";
...
export default function NumberGenerator({
  ...
  const kl8Validation =
    selectedType === "kl8"
      ? validateKL8BetLimit(mainNumbers.length, kl8NumberCount)
      : null;
  ...
  const saveNumbers = useCallback(() => {
    if (mainNumbers.length > 0) {
      ...
      if (selectedType === "kl8") {
        ...
        recordData.betAmount = calculateKL8BetAmount(
          mainNumbers.length,
          kl8NumberCount
        ); // 根据玩法和选择数量计算金额
      }
        ...
        mainTargetCount:
          selectedType !== "kl8"
            ? Array.isArray(config.mainCount)
              ...
              : config.mainCount
            : undefined,
        specialTargetCount: config.specialCount
        ...
        <CardContent className="space-y-6">
          <div className="space-y-4">
              ...
              <div className="bg-linear-to-r from-orange-50 to-red-50 dark:from-orange-900/20 dark:to-red-900/20 p-4 rounded-xl border border-orange-200/30 dark:border-orange-800/30">
                ...
                {kl8Validation && !kl8Validation.isValid && (
                  <div className="mt-2 text-xs text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-3 py-2 rounded-lg border border-red-200 dark:border-red-800">
                    <span className="font-medium">⚠️ 警告:</span>{" "}
                    {kl8Validation.errorMessage}
                  </div>
                )}
                ...
                <div className="mt-2 text-xs text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 px-3 py-2 rounded-lg border border-blue-200 dark:border-blue-800">
                  ...
                  {mainNumbers.length > 0
                    ? mainNumbers.length === kl8NumberCount
                      ...
                      : ` 选择${
                          mainNumbers.length
                        }个号码投注"选${kl8NumberCount}"玩法，共${calculateKL8BetCount(
                          mainNumbers.length,
                          kl8NumberCount
                        )}注，投注金额为 ${formatCurrency(
                          calculateKL8BetAmount(
                            mainNumbers.length,
                            kl8NumberCount
                          )
                ...
                {/* 单注限额提醒 */}
                {mainNumbers.length > 0 &&
                  calculateKL8BetAmount(mainNumbers.length, kl8NumberCount) >=
                    15000 && (
            ...
            {(selectedType === "dlt" || selectedType === "ssq") && (
              <div className="bg-linear-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 p-4 rounded-xl border border-blue-200/30 dark:border-blue-800/30">
                    ...
                    {(() => {
                      ...
                      if (selectedType === "dlt") {
                        if (
                          ...
                          return `复式投注: 前${mainNumbers.length}后${
                            specialNumbers.length
                          }，共${betCount}注，${formatCurrency(
                            calculateDLTBetAmount(
                              mainNumbers.length,
                              specialNumbers.length
                            )
                        ...
                        if (
                          ...
                          return `复式投注: 红${mainNumbers.length}蓝${
                            specialNumbers.length
                          }，共${betCount}注，${formatCurrency(
                            calculateSSQBetAmount(
                              mainNumbers.length,
                ...
                {(() => {
                  let betAmount = 0;
                  if (selectedType === "dlt") {
                    betAmount = calculateDLTBetAmount(
                      mainNumbers.length,
                      specialNumbers.length
                    );
                  } else if (selectedType === "ssq") {
                    betAmount = calculateSSQBetAmount(
                      mainNumbers.length,
                      specialNumbers.length
                    );
                  }
            ...
            {!isMobile && (
              <>
                ...
                <NumberOperations
                  ...
                  saved={saved}
                  kl8NumberCount={
                    selectedType === "kl8" ? kl8NumberCount : undefined
                  }
            ...
            <NumberSelector
              ...
              setSpecialNumbers={setSpecialNumbers}
              kl8NumberCount={
                selectedType === "kl8" ? kl8NumberCount || 10 : undefined
              }
            />
            <FrequencyLegend />
          </div>
