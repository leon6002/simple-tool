...
// 计算快乐8投注金额
export const calculateKL8BetAmount = (selectedCount: number, gameType: number = 10): number => {
...
// 计算快乐8注数（复式玩法）
export const calculateKL8BetCount = (selectedCount: number, gameType: number = 10): number => {
...
// 计算大乐透投注金额
export const calculateDLTBetAmount = (mainCount: number, specialCount: number): number => {
  // 大乐透投注规则：
...
// 计算双色球投注金额
export const calculateSSQBetAmount = (mainCount: number, specialCount: number): number => {
  // 双色球投注规则：
...
// 计算投注注数
export const calculateBetCount = (selectedType: LotteryType, mainCount: number, specialCount: number): number => {
  switch (selectedType) {
    case 'dlt':
      if (mainCount < 5 || specialCount < 2) return 0;
...
// 验证KL8投注金额是否超过限制
export const validateKL8BetLimit = (selectedCount: number, gameType: number = 10): {
  isValid: boolean;
  betAmount: number;
  errorMessage?: string;
} => {
